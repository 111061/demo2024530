...
public interface Contract_Management_ScreenRepository extends JpaRepository<Contract_Management_Screen, Long> {
    ...
    @Query("SELECT c FROM Contract_Management_Screen c WHERE " +
            ...
            "(CAST(c.contractCode AS string) LIKE %:keyword% OR c.contractingCompany LIKE %:keyword% OR c.contractedCompany LIKE %:keyword% OR c.contractType LIKE %:keyword% OR c.ourPosition LIKE %:keyword% OR c.ourSalesRepresentative LIKE %:keyword% OR c.theirSalesRepresentative LIKE %:keyword% OR CAST(c.contractDate AS string) LIKE %:keyword% OR c.quotation LIKE %:keyword%)")
